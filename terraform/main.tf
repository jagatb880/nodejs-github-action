...
resource "aws_subnet" "public" {
  ...
  #cidrsubnet(ip_block, newbits, netnum)
  cidr_block              = cidrsubnet(aws_vpc.main.cidr_block, 8, 10 + count.index) # 10.10.11.0/24, 10.10.12.0/24
  map_public_ip_on_launch = true
  tags                    = { Name = "ngapro-public-${local.azs_names[count.index]}" }
}
...
resource "aws_internet_gateway" "main" {
  vpc_id = aws_vpc.main.id
  tags   = { Name = "ngapro-igw" }
}

resource "aws_eip" "main" {
  count      = local.azs_count
  depends_on = [aws_internet_gateway.main]
  tags       = { Name = "ngapro-eip-${local.azs_names[count.index]}" }
}
...
resource "aws_iam_role" "ecs_node_role" {
  name_prefix        = "ngapro-ecs-node-role"
...
resource "aws_iam_instance_profile" "ecs_node" {
  name_prefix = "ngapro-ecs-node-profile"
  path        = "/ecs/instance/"
  role        = aws_iam_role.ecs_node_role.name
}
...
resource "aws_security_group" "ecs_node_sg" {
  name_prefix = "ngapro-ecs-node-sg-"
  vpc_id      = aws_vpc.main.id
...
resource "aws_iam_role" "ecs_task_role" {
  name_prefix        = "ngapro-ecs-task-role"
  assume_role_policy = data.aws_iam_policy_document.ecs_task_doc.json
}

resource "aws_iam_role" "ecs_exec_role" {
  name_prefix        = "ngapro-ecs-exec-role"
  assume_role_policy = data.aws_iam_policy_document.ecs_task_doc.json
}
